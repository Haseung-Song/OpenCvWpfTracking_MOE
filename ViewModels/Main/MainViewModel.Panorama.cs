...
namespace OpenCvWpfTracking.ViewModels.Main
{
    public partial class MainViewModel
    {
        ...
        public IList<string> PanoramaCaptureRangeOptions => PanoramaCaptureRangeOptionItems;
        ...
        public int SelectedPanoramaCaptureOptionIndex
        {
            ...
            set
            {
                ...
                if (_selectedPanoramaCaptureOptionIndex == safeValue) return;
        ...
        public int SelectedPanoramaRowCount => _selectedPanoramaCaptureOptionIndex + 1;
        public int SelectedPanoramaTotalFrameCount => SelectedPanoramaRowCount * PanoramaCaptureFrameCount;
        public string SelectedPanoramaCaptureSummary =>
            "TILT " + SelectedPanoramaTiltRangeDegrees + "° / " + SelectedPanoramaTotalFrameCount + " FRAMES";
        ...
        /// <summary>
        /// 2026-08-18: MOE EO 자동 파노라마 촬영 전 필수 상태를 검사한다.
        /// null이면 촬영 가능하며, 문자열이면 사용자에게 표시할 차단 사유다.
        /// </summary>
        public string GetPanoramaCaptureBlockReason()
        {
            if (!_controlAgentTcpService.IsConnected)
            {
            ...
            try
            {
                BuildPanoramaTiltTargets(_selectedPanoramaCaptureOptionIndex, _currentTilt);
            }
        ...
        private static IList<double> BuildPanoramaTiltTargets(int optionIndex, double startTilt)
        {
            ...
            if (targets.Exists(target => target < -90.0 || target > 90.0))
            {
                throw new InvalidOperationException(
                    ...
                    "START=" + startTilt.ToString("F2") + "°, TARGET=" +
                    string.Join(", ", targets.ConvertAll(target => target.ToString("F2") + "°")));
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            int selectedOptionIndex = _selectedPanoramaCaptureOptionIndex;
            IList<double> tiltTargets = BuildPanoramaTiltTargets(selectedOptionIndex, originalTilt);
            int selectedTiltRangeDegrees = (selectedOptionIndex + 1) * 12;
            int totalFrameCount = tiltTargets.Count * PanoramaCaptureFrameCount;
             ...
             * 자동 파노라마 촬영에서는 안정적인 정지를 위해 Speed 15를 고정 사용한다.
            ...
            ConsoleLogHelper.Info(
                ...
                "OPTION=" + (selectedOptionIndex + 1) + " / TILT_RANGE=" + selectedTiltRangeDegrees + "deg" +
                " / ROWS=" + tiltTargets.Count + " / COUNT_PER_ROW=" + PanoramaCaptureFrameCount +
                " / TOTAL_COUNT=" + totalFrameCount + " / STEP=10deg / TILT_TARGETS=" +
            ...
            try
            {
                     ...
                     rowIndex++)
                {
                    ...
                    double targetTilt = tiltTargets[rowIndex];
