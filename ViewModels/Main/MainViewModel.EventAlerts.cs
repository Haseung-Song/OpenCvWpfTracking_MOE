...
namespace OpenCvWpfTracking.ViewModels.Main
{
    public partial class MainViewModel
    {
        ...
        // 2026-09-09: Key = RTSP channel + ObjectId + ClassIndex.
            ...
            DateTime receiveTime)
        {
            ...
            foreach (AiDetectionBox box in boxes)
            {
                ...
                seenKeys.Add(key);
                string detectionType = GetAiEventDetectionType(box);
                if (_activeAiEvents.TryGetValue(key, out AiEventTrack existingTrack))
                {
                    ...
                    existingTrack.Event.UpdateDetectionType(detectionType);
                    existingTrack.Event.UpdateAiSnapshot(box.NormalizedConfidence * 100.0, box.Width, box.Height);
                ...
                FireEventRecord aiEvent = new FireEventRecord(
                    _nextAiEventId++, receiveTime, null, camera, detectionType,
                    (box.NormalizedConfidence * 100).ToString("F1", CultureInfo.InvariantCulture) + "%",
                    1, Math.Max(0, box.Width), Math.Max(0, box.Height),
                    Math.Max(0, box.Width) * (double)Math.Max(0, box.Height), "AI AGENT", "ACTIVE");
                if (_isAiCsvHistoryLoaded) aiEvent.MarkLiveAfterCsvLoad();
                _activeAiEvents[key] = new AiEventTrack
                {
                    RtspIndex = result.RtspIndex, ObjectId = box.ObjectId,
                    ClassIndex = box.ClassIndex, LastSeen = receiveTime, Event = aiEvent
                };
                ...
                ConsoleLogHelper.Warning("AI EVENT", "ACTION=CREATE / EVENT_ID=" + aiEvent.EventId +
                    " / CAMERA=" + camera + " / OBJECT_ID=" + box.ObjectId + " / CLASS=" + detectionType);
            }

            foreach (KeyValuePair<string, AiEventTrack> pair in _activeAiEvents
                .Where(item => item.Value.RtspIndex == result.RtspIndex && !seenKeys.Contains(item.Key) &&
                    (receiveTime - item.Value.LastSeen).TotalMilliseconds >= AiDisplayHoldMilliseconds).ToList())
            {
                pair.Value.Event.MarkCleared(receiveTime);
                AppendFireEventAudit(pair.Value.Event, "CLEARED");
                _activeAiEvents.Remove(pair.Key);
                ConsoleLogHelper.State("AI EVENT", "ACTION=CLEAR / EVENT_ID=" + pair.Value.Event.EventId +
