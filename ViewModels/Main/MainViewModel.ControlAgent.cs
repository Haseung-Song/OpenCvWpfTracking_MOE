...
{
    /// <summary>
    /// 환경부 Web Agent TCP 연결, Packet 수신/파싱과
    /// 장비 상태 반영을 관리한다.
    ///
    ...
    {
        #region [Control Agent Communication]

        #region [Control Agent Connect]
        ...
        /// </summary>
        private async Task<bool> ConnectControlAgentAsync()
        {
            ...
            */
            ...
            {
                bool result =
                    await _controlAgentTcpService.ConnectAsync(
                        targetIp,
                        targetPort);
            ...
            _ = Task.Run(async () =>
            {
                ...
                try
                {
                    while (_isDeviceConnectionRequested &&
                           ...
                           !_controlAgentTcpService.IsConnected)
                    {
                        retryCount++;
        ...
        /// </summary>
        private void OnControlAgentMessageReceived(
            byte[] data,
        ...
        #region [Control Agent Packet Handling]
        ...
        private void HandleLaPacket(LaResponsePacket packet)
        {
            ...
            {
                ConsoleLogHelper.PrintLine();
                Console.WriteLine("[CONTROL AGENT PACKET] Invalid Checksum");
                ConsoleLogHelper.PrintLine();
                return;
            }
            ...
            switch (packet.Function)
            {
                case 0x01:
                    ...
                    ConsoleLogHelper.PrintLine();
                    Console.WriteLine("[CONTROL AGENT PACKET] [Pan] / [Tilt] / [Zoom] / [Focus] Status");
                    Console.WriteLine();
                ...
                case 0x04:
                    ...
                    ConsoleLogHelper.PrintLine();
                    Console.WriteLine("[CONTROL AGENT PACKET] [LRF] Distance Packet");
                    Console.WriteLine();
                ...
                default:
                    ...
                    ConsoleLogHelper.PrintLine();
                    Console.WriteLine($"[CONTROL AGENT PACKET] Unknown Function: 0x{packet.Function:X2}");
                    Console.WriteLine();
        ...
        #region [Control Agent Log Helpers]
        ...
        #region [Control Agent Packet Parsing]
            ...
            bool printLog)
        {
            ...
            */
            ...
            ///
            /// Web Agent가 -180 명령 후에도 상태값을 +180으로 반환할 수 있으므로,
        ...
        ///
        /// Control Agent TCP 수신 이벤트는 Receive Thread에서 호출되므로
        ...
        /// 수신되면 ParseLaIrCameraStatusPacket이 실측값으로 다시 덮어쓴다.
        /// </summary>
            ...
            bool printLog)
        {
                ...
                packet.RawData.Length < 12)
            {
                ...
                {
                    Console.WriteLine(
                        "[CONTROL AGENT IR STATUS] Invalid Packet Length : " +
                ...
                !isFocusInRange)
            {
                ...
                Console.WriteLine(
                    "[CONTROL AGENT IR STATUS] Invalid Position Range");

                Console.WriteLine(
                    $"[CONTROL AGENT IR STATUS] Zoom  : {irZoomPosition} / 1000");

                Console.WriteLine(
                    $"[CONTROL AGENT IR STATUS] Focus : {irFocusPosition} / 1000");

                Console.WriteLine(
                    "[CONTROL AGENT IR STATUS RAW] " +
                    BitConverter
                        .ToString(
                            packet.RawData)
                        .Replace(
            ...
            Console.WriteLine(
                "[CONTROL AGENT PACKET] [IR] Zoom / Focus Status");

            Console.WriteLine();

            Console.WriteLine(
                $"[CONTROL AGENT IR STATUS] [Zoom]  : {_currentIrZoom} / 1000");

            Console.WriteLine(
                $"[CONTROL AGENT IR STATUS] [Focus] : {_currentIrFocus} / 1000");
            ...
            bool printLog)
        {
            ...
            Console.WriteLine(
                "[CONTROL AGENT PACKET] Unconfirmed Extended Status : 0xA1");

            Console.WriteLine();

            Console.WriteLine(
                "[CONTROL AGENT EXT STATUS RAW] " +
                BitConverter
                    .ToString(
                        packet)
                    .Replace(
            ...
            Console.WriteLine(
                "[CONTROL AGENT EXT STATUS] CURRENT STATUS not updated.");
