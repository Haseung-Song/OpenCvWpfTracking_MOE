...
namespace OpenCvWpfTracking.ViewModels.Main
{
    ...
    public partial class MainViewModel
    {
        ...
        {
            // 환경부 전용 V1에서는 HOME 기능 자체를 제공하지 않는다.
            // UI 외부에서 Command가 직접 실행되더라도 장비 명령을 송신하지 않는다.
            if (!IsHomeZeroFeatureEnabled)
            {
                return;
            }
        ...
        /// <summary>
        /// 환경부 V1에서는 PAN ZERO를 지원하지 않는다.
        /// </summary>
        private Task SetPanZeroAsync()
        {
            return Task.CompletedTask;
        }

        /// <summary>
        /// 환경부 V1에서는 TILT ZERO를 지원하지 않는다.
        /// </summary>
        private Task SetTiltZeroAsync()
        {
            return Task.CompletedTask;
        }
        ...
        private async Task MovePanAbsoluteFromInputAsync()
        {
            ...
            // 실장비 Web Agent는 위치 속도와 절대좌표 명령을 연속 전송하면
            // 앞선 설정 명령을 처리하기 전에 다음 Packet을 받을 수 있다.
            await Task.Delay(60);
        ...
        private async void MoveTiltAbsoluteFromInput()
        {
            ...
            // Pan과 동일하게 속도 설정 적용 시간을 확보한 뒤 목표를 보낸다.
            await Task.Delay(60);
        ...
        /// </summary>
        private async void AddOrUpdateLaPresetPoint()
        {
            ConsoleLogHelper.Command(
                "PRESET L",
            ...
            CancellationToken cancellationToken)
        {
            ...
            // 프리셋 직접 이동도 수동 Absolute·파노라마와 같은 장비 명령
            // 처리 간격을 사용해 연속 Packet 유실을 방지한다.
        ...
        /// <summary>
        /// 환경부 PRESET 이동에 사용할 Pan / Tilt 위치 속도를 장비에 적용한다.
