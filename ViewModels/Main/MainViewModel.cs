...
namespace OpenCvWpfTracking.ViewModels.Main
{
    ...
    public partial class MainViewModel : INotifyPropertyChanged
    {
        ...
        #region [Environment RTSP Source Fields]

        /*
         * TODO(CONFIG-NEXT):
         * 장비 주소와 인증정보가 최종 확정되면 App.config 또는 별도 설정 파일로 이동한다.
         * 환경부 운용 기본값만 상수로 제공하고,
         * 실제 연결 주소는 통신 설정 탭의 TextBox에서 수정할 수 있다.
        ...
        /// <summary>
        /// 환경부 실장비 주간(EO) PTZ 카메라 RTSP 기본값
        /// </summary>
        ...
        #region [Control Agent Communication Fields]

        /// <summary>
        /// [Control Agent] 제어 [TCP] 통신 서비스 객체
        ///
        /// 환경부 Web Agent에 TCP로 연결하며,
        /// 모든 PTZF 제어 명령은 이 연결을 통해 송수신한다.
        /// </summary>
        private readonly TcpClientService _controlAgentTcpService;
        ...
        /// <summary>
        /// 환경부 전용 V1에서는 장비가 HOME / PAN ZERO / TILT ZERO를
        /// 지원하지 않으므로 관련 UI와 연결 직후 자동 HOME을 모두 비활성화한다.
        ///
        /// Rooftop / LA 장비 기능이 포함된 V2는 해당 기능을 그대로 유지한다.
        /// </summary>
        private static readonly bool IsHomeZeroFeatureEnabled =
            false;
        ...
        /// <summary>
        /// 환경부 PRESET 단일 MOVE TO PRESET 작업 취소 토큰
        /// </summary>
        ...
        public MainViewModel()
        {
            ...
            /// </summary>
            _controlAgentTcpService = new TcpClientService();

            /// <summary>
            /// [TORUSS] 제어 명령 서비스 생성
            /// </summary>
            _controlCommandService =
                new ControlCommandService(
                    _controlAgentTcpService);
            ...
            /// [OnControlAgentMessageReceived] 함수가 호출된다.
            /// </summary>
            _controlAgentTcpService.MessageReceived +=
                OnControlAgentMessageReceived;

            /// <summary>
            /// [Control Agent] 서버가 연결을 종료한 경우
            /// 장비 연결 요청 상태가 유지되어 있으면 자동 재연결을 시작한다.
            /// </summary>
            _controlAgentTcpService.ConnectionClosed +=
                OnControlAgentConnectionClosed;
        ...
        {
            // 환경부 실장비 Web Agent IP
            ControlAgentIp =
                "192.168.20.161";

            // 환경부 실장비 Web Agent Port
        ...
        /// <summary>
        /// 환경부 EO / IR RTSP 기본값 초기화
        ///
        /// 통신 설정 탭의 EO / IR RTSP 입력칸에 표시하며,
        /// 운용자가 주소를 수정한 후 다시 연결할 수 있다.
        /// </summary>
