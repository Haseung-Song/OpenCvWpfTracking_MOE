            ...
            SmokeDiagnosticCapture diagnostic)
        {
            ...
            foreach (Rect candidate in candidates)
            {
                ...
                foreach (SmokeCandidateTrack track in _tracks)
                {
                    ...
                    if (highZoomMode && !isInfrared && score < 0.08 && track.MissingFrames <= 12)
                    {
                        ...
                        double areaRatio = Math.Max(track.Rectangle.Width * (double)track.Rectangle.Height, candidate.Width * (double)candidate.Height) /
                            Math.Max(1.0, Math.Min(track.Rectangle.Width * (double)track.Rectangle.Height, candidate.Width * (double)candidate.Height));
                        ...
                        if (sourceDx <= Math.Max(10.0, Math.Min(track.Rectangle.Width, candidate.Width) * sourceXFactor) &&
                            sourceDy <= Math.Max(8.0, Math.Min(track.Rectangle.Height, candidate.Height) * sourceYFactor) && areaRatio <= 3.0 + 0.25 * highZoomStrength)
                        {
                            score = 0.085;
                        }
                ...
                else { bestTrack.FreshEvidenceAge++; }
            ...
            foreach (SmokeCandidateTrack track in _tracks)
            {
                ...
                bool accumulatedDirectionAccepted = highZoomMode && !isInfrared && track.RecentVerticalMotionCount >= 10 &&
                    track.RecentUpwardSamples >= 3 && track.RecentUpwardSamples >= track.RecentDownwardSamples + 2;
                bool heatShimmerOscillation = highZoomMode && !isInfrared && track.RecentVerticalMotionCount >= 12 &&
                    track.RecentDirectionChanges >= 7 && Math.Abs(track.RecentVerticalNetMotion) <= track.RecentVerticalAbsoluteMotion * 0.12 &&
                ...
                bool maturePlumeEvidence =
                    ...
                    HasMaturePlumeEvidence(
                        ...
                        frameWidth,
                        frameHeight);
                bool directionAccepted = (baseDirectionAccepted || accumulatedDirectionAccepted || maturePlumeEvidence) && !heatShimmerOscillation;
                ...
                if (rigidMovingObject || roadTrafficAggregate ||
                    weakBottomBoundary || hyperDynamicBackground || heatShimmerOscillation || track.FreshEvidenceAge > 45)
                {
