...
namespace OpenCvWpfTracking.Services.Video
{
    ...
    public sealed class EoPanoramaStitchingService
    {
            ...
            out bool usedFixedAngleFallback)
        {
            ...
            if (frameRows == null || frameRows.Count < 1 || frameRows[0] == null ||
                frameRows[0].Count < 24 ||
            ...
            int frameCount = frameRows[0].Count;
            ...
            for (int rowIndex = 0; rowIndex < frameRows.Count - 1; rowIndex++)
            {
                ...
                int stableOverlap;
                int stableVerticalOffset;
                EstimateStableColumnPairGeometry(upperFrames, lowerFrames, nominalOverlap,
                    out stableOverlap, out stableVerticalOffset);
            ...
            try
            {
                ...
                System.Threading.Tasks.Parallel.For(0, frameCount,
                    ...
                    index =>
                {
                    List<Mat> normalizedRows = frameRows.Select(row => row[index].Clone()).ToList();
                    ...
                    try
                    {
                        for (int rowIndex = 1; rowIndex < frameRows.Count; rowIndex++)
                        {
                            Mat combined = MergeTiltRowIntoColumn(
                                column, normalizedRows[rowIndex],
                                stableOverlaps[rowIndex - 1], stableVerticalOffsets[rowIndex - 1]);
                            column.Dispose();
                            column = combined;
                        }
            ...
            int verticalOffset)
        {
            ...
            using (Mat shiftedNext = ShiftRowVertically(nextRow, verticalOffset))
            {
                ...
                return MergeRowsOnAdaptiveHorizontalSeam(accumulatedUpper, shiftedNext, safeOverlap);
            }
        }
